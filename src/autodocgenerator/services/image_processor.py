...
class ImageProcessingSettings:
    """Configuration for transfer cropping and receipt preservation."""
    ...
    def __post_init__(self) -> None:
        if self.border_width_pt < 0:
            raise ValueError("border_width_pt must be greater than or equal to zero.")
        ...
        ratio_fields = {
            "transfer_initial_top_trim_ratio": self.transfer_initial_top_trim_ratio,
            "transfer_initial_bottom_trim_ratio": self.transfer_initial_bottom_trim_ratio,
            "transfer_initial_left_trim_ratio": self.transfer_initial_left_trim_ratio,
            "transfer_initial_right_trim_ratio": self.transfer_initial_right_trim_ratio,
            "transfer_extra_top_crop_ratio": self.transfer_extra_top_crop_ratio,
        ...
        for field_name, value in ratio_fields.items():
            if not 0 <= value < 1:
                raise ValueError(f"{field_name} must be between 0 and less than 1.")

        if not 0 <= self.ocr_minimum_word_confidence <= 100:
            raise ValueError("ocr_minimum_word_confidence must be between 0 and 100.")

        if self.ocr_page_segmentation_mode < 0:
            raise ValueError("ocr_page_segmentation_mode must not be negative.")
...
class ImageProcessor:
    """
    Prepare source files for Word generation.
    ...
    - add a 1.5 pt black border by default;
    - never force a square.

    REAL RECEIPT image:
    - never crop, pad, resize, or force a square;
    - add a 1.5 pt black border by default.

    REAL RECEIPT PDF:
    - render every page as a lossless PNG;
    - preserve page order and original page aspect ratio;
    - never run the transfer crop on rendered pages;
    - add a 1.5 pt black border to every rendered page.
    """
    ...
    def __init__(self, *, settings: ImageProcessingSettings | None = None) -> None:
        ...
        for source_image in source_images:
            if source_image.is_pdf_receipt:
                try:
                    ...
                    source_image.warnings.append(f"Pemrosesan PDF gagal: {error}")
            ...
            try:
                ...
                source_image.requires_review = True
                source_image.warnings.append(f"Pemrosesan gambar gagal: {error}")
    ...
    def process_single(
        ...
        *,
        ...
        if not source_path.is_file():
            raise ImageProcessingError(f"File gambar tidak ditemukan: {source_path}")
        ...
        try:
            with Image.open(source_path) as source:
                ...
                bordered = self._add_border(
                    cropped,
                    dpi=self._settings.output_dpi,
                )
                bordered.save(
                    output_path,
                    format="JPEG",
    ...
    def _process_pdf_receipt(
        ...
        *,
        ...
        if not source_path.is_file():
            raise ImageProcessingError(f"File PDF tidak ditemukan: {source_path}")
        ...
        try:
            ...
            for page_index in range(document.page_count):
                ...
                sequence_number = starting_sequence_number + page_index
                ...
                pages.append(
                    SourceImage(
                        ...
                        transaction_datetime=source_image.transaction_datetime,
    ...
    def _prepare_transfer_proof(self, image: Image.Image) -> Image.Image:
    ...
    def _adaptive_transfer_crop(self, image: Image.Image) -> Image.Image:
    ...
    def _detect_semantic_transaction_box(
        ...
        title_candidates = [
            line
            for line in lines
            if self._line_contains_words(line, ("transfer", "dana"))
        ]

        if not title_candidates:
            return None

        title_line = min(title_candidates, key=lambda line: line.top)
        ...
        authorization_candidates = [
            ...
            and self._line_contains_any_word(
                line,
                ("diotorisasi", "otorisasi", "authorized"),
            )
        ]
        ...
        if authorization_candidates:
            ...
            fallback_candidates = [
                ...
                if line.top >= title_line.top
                and (
                    self._line_contains_any_word(line, ("dibuat", "created"))
                    or re.search(
        ...
        top_padding = max(
            2,
            round(image.height * self._settings.title_padding_ratio),
        )
        bottom_padding = max(
            2,
            round(image.height * self._settings.bottom_padding_ratio),
        )

        top = max(0, title_line.top - top_padding)
        bottom = min(image.height, bottom_line.bottom + bottom_padding)

        if bottom <= top or bottom - top < image.height * 0.08:
            return None

        return (0, top, image.width, bottom)

    def _read_ocr_lines(self, image: Image.Image) -> list[_OCRLine]:
        config = (
            "--oem 3 "
        ...
        grouped_indexes: dict[tuple[int, int, int, int], list[int]] = defaultdict(list)
        ...
        for index, raw_text in enumerate(text_values):
            ...
            if confidence < self._settings.ocr_minimum_word_confidence:
                continue

            key = (
                self._safe_data_integer(data, "page_num", index),
                self._safe_data_integer(data, "block_num", index),
                self._safe_data_integer(data, "par_num", index),
                self._safe_data_integer(data, "line_num", index),
            )
        ...
        for indexes in grouped_indexes.values():
            ...
            left = min(
                self._safe_data_integer(data, "left", index)
                for index in indexes
            )
            top = min(
                self._safe_data_integer(data, "top", index)
                for index in indexes
            )
            right = max(
                self._safe_data_integer(data, "left", index)
                + self._safe_data_integer(data, "width", index)
                for index in indexes
            )
            bottom = max(
                self._safe_data_integer(data, "top", index)
                + self._safe_data_integer(data, "height", index)
                for index in indexes
            )
    ...
    def _line_contains_words(
        ...
        return all(self._line_contains_any_word(line, (word,)) for word in required_words)
    ...
    def _line_contains_any_word(
        ...
        for expected_word in expected_words:
            ...
            for actual_word in actual_words:
                ...
                if similarity >= self._settings.ocr_fuzzy_match_threshold:
                    return True
    ...
    @staticmethod
    def _parse_confidence(values: object, index: int) -> float:
        try:
    ...
    @staticmethod
    def _safe_data_integer(data: dict[str, list[object]], key: str, index: int) -> int:
        values = data.get(key, [])
    ...
    def _build_transfer_output_path(
        ...
        *,
        ...
        safe_stem = self._sanitize_filename(source_image.path.stem)
        return processed_directory / f"{sequence_number:03d}_{safe_stem}.jpg"
    ...
    def _build_pdf_output_path(
        ...
        *,
        ...
        return processed_directory / (
            f"{sequence_number:03d}_{safe_stem}_page_{page_number:03d}.png"
        )
    ...
    def _add_border(
        ...
        *,
        ...
        border_width_px = self._border_width_pixels(dpi=dpi)
